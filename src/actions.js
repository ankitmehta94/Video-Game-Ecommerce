...
export function fetchGenres() {
    return dispatch => {
        fetch('https://igdbcom-internet-game-database-v1.p.mashape.com/genres/?fields=*&limit=40',{headers: {'X-Mashape-Key': 'PZzFnwlUa7mshIR29b7zMporUI45p16v4AejsnayIdsA9wY2nj','Accept': 'application/json'}})
            .then(res => res.json()).then(data =>dispatch(genresReceived(data)))
    }
}
export function fetchPopularGames() {
    return dispatch => {
        fetch('https://igdbcom-internet-game-database-v1.p.mashape.com/games/?fields=name,cover,aggregated_rating&order=popularity:desc',{headers: {'X-Mashape-Key': 'PZzFnwlUa7mshIR29b7zMporUI45p16v4AejsnayIdsA9wY2nj','Accept': 'application/json'}})
