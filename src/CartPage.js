...
class CartPage extends  React.Component {
           ...
           return (<Item key={game.id} >
                ...
                <Item.Content verticalAlign='middle'>
                    ...
                    <Item.Meta className="full-width flex-row-space-between" >
                        ...
                        <Icon name='close' size="large" onClick={()=>{this.props.removeGameFromCart(game)}}/>
                    </Item.Meta>   
                </Item.Content>
            </Item>)
        }))
    ...
    renderPayment(pay) {
        return (
            <div className="flex-col-space-around full-height full-width">
    ...
    createPaymentList(){
        return paymentArray.map((pay)=>{
            return (
                <Menu.Item key={pay.name} name={pay.name} active={this.state.activePayType.name === pay.name}  onClick={()=>{this.setState({activePayType:pay})}} />
                )
        })
    }
...
function matchDispatchToProps(dispatch) {
    return bindActionCreators({removeGameFromCart:removeGameFromCart}, dispatch);
}
